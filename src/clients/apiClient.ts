...
export const hentListeMedArbeidsforhold = (customApiUrl?: string) => {
    const defaultApiUrl = `${Environment.apiUrl}/arbeidsforhold/forenklet/alle`;
    return hentJsonOgSjekkAuth(customApiUrl ?? defaultApiUrl);
};

export const hentDetaljertArbeidsforhold = (id: number, customApiUrl?: string) => {
    const defaultApiUrl = `${Environment.apiUrl}/arbeidsforhold/{id}`;
