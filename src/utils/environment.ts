...
class Environment {
  ...
  static settEnv = (miljo: Miljo) => {
    switch (miljo) {
      default:
      case "LOCAL":
        Environment.apiUrl = `http://localhost:8080/person/personopplysninger-api`;
        Environment.loginUrl = `https://loginservice-q.nav.no/login`;
        Environment.logoutUrl = `https://loginservice-q.nav.no/slo`;
        break;
      case "DEV":
        Environment.apiUrl = `https://www-q0.nav.no/person/personopplysninger-api`;
        Environment.loginUrl = `https://loginservice-q.nav.no/login`;
        Environment.logoutUrl = `https://loginservice-q.nav.no/slo`;
        break;
      case "PROD":
        Environment.apiUrl = `https://www.nav.no/person/personopplysninger-api`;
